...
public class StudentServiceImpl implements StudentService {
	...
	public void insert(StudentBean bean) {
		Student student = studentDAO.update(bean.getId());
		if (student != null) {
	...
	public List<StudentBean> fetchData() {
		...
		List<StudentBean> list1 = slist.stream().map(student -> {
			...
			return new StudentBean(student.getId(), student.getName(), student.getAge(), student.getBranch(),
					student.getMarks().getMaths(), student.getMarks().getEnglish(), student.getMarks().getSocial(), sum,
					percentage);
